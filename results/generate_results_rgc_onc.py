...
ray.init()
dataset_rgc = sc.read("./Mouse_RGC/RGC_onc.h5ad")
# dataset_popv = sc.read("/Volumes/SSD/popv_immune.h5ad")
list_celltypes = dataset_rgc.obs['clusters'].unique().tolist()
list_celltypes = list(filter(lambda x: x not in ['46', 'Unassigned'], list_celltypes))
...
if os.path.exists('./results_rgc_onc/g.pkl'):
    with open('./results_rgc_onc/g.pkl', 'rb') as f:
        g = pickle.load(f)
else:
    with open('./results_rgc_onc/g.pkl', 'wb') as f:
...
if os.path.exists('./results_rgc_onc/g.png'):
    pass
else:
    ...
    ig.plot(g, './results_rgc_onc/g.png', layout=layout, bbox=(0, 0, 5000, 500), vertex_label=g.vs['name'], vertex_label_size=10, vertex_label_dist=1.5, vertex_size=10, vertex_color='white', edge_color='grey', margin=50)
# dist_df = helper_fns.get_dist_df(list_num_ct=list_num_ct, g=g)
# D = torch.tensor(dist_df.values, dtype=float)

if os.path.exists('./results_rgc_onc/dist_df.csv'):
    dist_df = pd.read_csv('./results_rgc_onc/dist_df.csv', index_col=0)
else:
    dist_df = helper_fns.get_dist_df(list_num_ct=list_num_ct, g=g)
    dist_df.to_csv('./results_rgc_onc/dist_df.csv')
D = torch.tensor(dist_df.values, dtype=float)

# Chcek if the cv object is saved
if os.path.exists("./results_rgc_onc/cv.pkl"):
    cv = joblib.load("./results_rgc_onc/cv.pkl")
else:
    ...
    joblib.dump(cv, "./results_rgc_onc/cv.pkl")
...
def get_results(i, cv, dataset_rgc, dataset_popv, encoders, D, list_num_ct):
    ...
    if not os.path.exists("./results_rgc_onc/models/PCA"):
        os.makedirs("./results_rgc_onc/models/PCA")
    joblib.dump(pca, "./results_rgc_onc/models/PCA/pca_fold_{}.pkl".format(i))
    print('PCA Done')
    ...
    if not os.path.exists("./results_rgc_onc/models/Proto_Net_disto_pl"):
        os.makedirs("./results_rgc_onc/models/Proto_Net_disto_pl")
    torch.save(model.state_dict(), "./results_rgc_onc/models/Proto_Net_disto_pl/model_fold_{}.pt".format(i))
    ...
    if not os.path.exists("./results_rgc_onc/models/Proto_Net_disto"):
        os.makedirs("./results_rgc_onc/models/Proto_Net_disto")
    torch.save(model.state_dict(), "./results_rgc_onc/models/Proto_Net_disto/model_fold_{}.pt".format(i))
    preds, embeddings = model(torch.tensor(dataset_rgc[cv[i]['test']].X))
    # Apply softmax to the predictions
    preds = torch.nn.functional.softmax(preds, dim=1).detach().cpu().numpy()
    preds_labels = encoder_celltype.inverse_transform(preds.argmax(axis=1))
    ...
    if not os.path.exists("./results_rgc_onc/models/Proto_Net_pl"):
        os.makedirs("./results_rgc_onc/models/Proto_Net_pl")
    torch.save(model.state_dict(), "./results_rgc_onc/models/Proto_Net_pl/model_fold_{}.pt".format(i))
    preds, embeddings = model(torch.tensor(dataset_rgc[cv[i]['test']].X))
    # Apply softmax to the predictions
    preds = torch.nn.functional.softmax(preds, dim=1).detach().cpu().numpy()
    preds_labels = encoder_celltype.inverse_transform(preds.argmax(axis=1))
    ...
    if not os.path.exists("./results_rgc_onc/models/Proto_Net"):
        os.makedirs("./results_rgc_onc/models/Proto_Net")
    torch.save(model.state_dict(), "./results_rgc_onc/models/Proto_Net/model_fold_{}.pt".format(i))
    preds, embeddings = model(torch.tensor(dataset_rgc[cv[i]['test']].X))
    # Apply softmax to the predictions
    preds = torch.nn.functional.softmax(preds, dim=1).detach().cpu().numpy()
    preds_labels = encoder_celltype.inverse_transform(preds.argmax(axis=1))
    ...
    if not os.path.exists("./results_rgc_onc/models/Net"):
        os.makedirs("./results_rgc_onc/models/Net")
    torch.save(model.state_dict(), "./results_rgc_onc/models/Net/model_fold_{}.pt".format(i))
    ...
    if not os.path.exists("./results_rgc_onc/models/logistic_regression"):
        os.makedirs("./results_rgc_onc/models/logistic_regression")
    joblib.dump(model, "./results_rgc_onc/models/logistic_regression/model_fold_{}.pkl".format(i))
    ...
    if not os.path.exists("./results_rgc_onc/models/KNN"):
        os.makedirs("./results_rgc_onc/models/KNN")
    joblib.dump(model, "./results_rgc_onc/models/KNN/model_fold_{}.pkl".format(i))
...
# results.to_csv('results_rgc_onc.csv')
...
preds_and_score_overall.to_csv('preds_and_score_overall_rgc_onc.csv')

# Save results (tuple)
# with open('./results_rgc_onc/results_rgc_onc.pickle', 'wb') as handle:
