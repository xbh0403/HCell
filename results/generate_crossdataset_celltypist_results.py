...
@ray.remote
def get_results(i, dataset_popv, dataset_celltypist):
    print("Fold: {}".format(i))
    ...
    #     pca = joblib.load(f)
    pca = joblib.load("./results_celltypist/models/PCA/pca_fold_{}.pkl".format(i))
    ...
    model.load_state_dict(torch.load("./results_celltypist/models/Proto_Net_disto_pl/model_fold_{}.pt".format(i)))
    ...
    preds_df['true'] = dataset_popv.obs['cell_type'].tolist()
    ...
    results_cross_dataset = pd.concat([results_cross_dataset, pd.DataFrame({'fold': i, 'model': 'Proto_Net+disto_pl',
        'accuracy': accuracy_score(dataset_popv.obs['cell_type'], preds_df['pred_0']),
        'recall': recall_score(dataset_popv.obs['cell_type'], preds_df['pred_0'], average='weighted'),
        'f1': f1_score(dataset_popv.obs['cell_type'], preds_df['pred_0'], average='weighted')}, index=[0])], axis=0)

    print('Proto_Net+disto_pl Done')

    print('Proto_Net+disto Start')
    model = m.init_model(mode='Proto_Net', D=dist_df, embedding_dim=16, num_celltypes=list_num_ct)
    model.load_state_dict(torch.load("./results_celltypist/models/Proto_Net_disto/model_fold_{}.pt".format(i)))
    ...
    model = m.init_model(mode='Proto_Net', D=dist_df, embedding_dim=16, num_celltypes=list_num_ct)
    model.load_state_dict(torch.load("./results_celltypist/models/Proto_Net_pl/model_fold_{}.pt".format(i)))
    ...
    model = m.init_model(mode='Proto_Net', D=dist_df, embedding_dim=16, num_celltypes=list_num_ct)
    model.load_state_dict(torch.load("./results_celltypist/models/Proto_Net/model_fold_{}.pt".format(i)))
    ...
    model.load_state_dict(torch.load("./results_celltypist/models/Net/model_fold_{}.pt".format(i)))
    ...
    for k in range(preds_df.shape[0]):
        if preds_df['true'].tolist()[k] == preds_df['pred_0'].iloc[k]:
            ...
            match_index.append(5)
    preds_df['match_index'] = match_index
    results_cross_dataset_dict['Net_{}'.format(i)] = preds_df
    ...
    #     model = pickle.load(f)
    model = joblib.load('./results_celltypist/models/logistic_regression/model_fold_{}.pkl'.format(i))
    
    # preds = model.predict(dataset_popv.X)
    preds = model.predict_proba(dataset_popv.X)
    preds_df = pd.DataFrame()
    for j in range(5):
    ...
    #     model = pickle.load(f)
    model = joblib.load('./results_celltypist/models/KNN/model_fold_{}.pkl'.format(i))
...
cv0 = get_results.remote(0, dataset_popv, dataset_celltypist)
cv1 = get_results.remote(1, dataset_popv, dataset_celltypist)
cv2 = get_results.remote(2, dataset_popv, dataset_celltypist)
cv3 = get_results.remote(3, dataset_popv, dataset_celltypist)
cv4 = get_results.remote(4, dataset_popv, dataset_celltypist)
# cv2 = cv.remote(2)
# cv3 = cv.remote(3)
# cv4 = cv.remote(4)
...
# Save results (tuple)
with open('./results_celltypist/results_pred_top5_true_model_crossdataset_celltypist.pickle', 'wb') as handle:
