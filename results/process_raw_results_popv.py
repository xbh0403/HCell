...
results = joblib.load('./results_popv/results_popv.pickle')
df_list = []
dict_list = []
...
results_df.to_csv('./results_popv/results_popv.csv')
...
for d in dict_list:
    for k, v in d.items():
        results_dict[k] = v
with open('./results_popv/results_popv_dict.pickle', 'wb') as handle:
...
results_df = pd.concat(df_list)
results_df.to_csv('./results_popv/results_popv_celltypist.csv')
...
for d in dict_list:
    for k, v in d.items():
        results_dict[k] = v
with open('./results_popv/results_popv_celltypist_dict.pickle', 'wb') as handle:
    pickle.dump(results_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)