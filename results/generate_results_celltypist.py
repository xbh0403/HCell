...
if os.path.exists('./results_celltypist/g.pkl'):
    with open('./results_celltypist/g.pkl', 'rb') as f:
        g = pickle.load(f)
else:
    with open('./results_celltypist/g.pkl', 'wb') as f:
...
if os.path.exists('./results_celltypist/g.png'):
    pass
else:
    ...
    ig.plot(g, './results_celltypist/g.png', layout=layout, bbox=(0, 0, 5000, 500), vertex_label=g.vs['name'], vertex_label_size=10, vertex_label_dist=1.5, vertex_size=10, vertex_color='white', edge_color='grey', margin=50)
# dist_df = helper_fns.get_dist_df(list_num_ct=list_num_ct, g=g)
# D = torch.tensor(dist_df.values, dtype=float)

if os.path.exists('./results_celltypist/dist_df.csv'):
    dist_df = pd.read_csv('./results_celltypist/dist_df.csv', index_col=0)
else:
    dist_df = helper_fns.get_dist_df(list_num_ct=list_num_ct, g=g)
    dist_df.to_csv('./results_celltypist/dist_df.csv')
D = torch.tensor(dist_df.values, dtype=float)

# Chcek if the cv object is saved
if os.path.exists("./results_celltypist/cv.pkl"):
    cv = joblib.load("./results_celltypist/cv.pkl")
else:
    train_indices, test_indices, cv = helper_fns.costumized_train_test_split(dataset_celltypist, cross_validation=True, k_fold=5)
    joblib.dump(cv, "./results_celltypist/cv.pkl")
...
def get_results(i, cv, dataset_celltypist, dataset_popv, encoders, D, list_num_ct):
    ...
    if not os.path.exists("./results_celltypist/models/PCA"):
        os.makedirs("./results_celltypist/models/PCA")
    joblib.dump(pca, "./results_celltypist/models/PCA/pca_fold_{}.pkl".format(i))
    print('PCA Done')
    ...
    if not os.path.exists("./results_celltypist/models/Proto_Net_disto_pl"):
        os.makedirs("./results_celltypist/models/Proto_Net_disto_pl")
    torch.save(model.state_dict(), "./results_celltypist/models/Proto_Net_disto_pl/model_fold_{}.pt".format(i))
    ...
    if not os.path.exists("./results_celltypist/models/Proto_Net_disto"):
        os.makedirs("./results_celltypist/models/Proto_Net_disto")
    torch.save(model.state_dict(), "./results_celltypist/models/Proto_Net_disto/model_fold_{}.pt".format(i))
    # preds, embeddings = model(torch.tensor(dataset_celltypist[cv[i]['test']].X))
    # # Apply softmax to the predictions
    # preds = torch.nn.functional.softmax(preds, dim=1).detach().cpu().numpy()
    # preds_labels = encoder_celltype.inverse_transform(preds.argmax(axis=1))
    ...
    if not os.path.exists("./results_celltypist/models/Proto_Net_pl"):
        os.makedirs("./results_celltypist/models/Proto_Net_pl")
    torch.save(model.state_dict(), "./results_celltypist/models/Proto_Net_pl/model_fold_{}.pt".format(i))
    # preds, embeddings = model(torch.tensor(dataset_celltypist[cv[i]['test']].X))
    # # Apply softmax to the predictions
    # preds = torch.nn.functional.softmax(preds, dim=1).detach().cpu().numpy()
    # preds_labels = encoder_celltype.inverse_transform(preds.argmax(axis=1))
    ...
    if not os.path.exists("./results_celltypist/models/Proto_Net"):
        os.makedirs("./results_celltypist/models/Proto_Net")
    torch.save(model.state_dict(), "./results_celltypist/models/Proto_Net/model_fold_{}.pt".format(i))
    # preds, embeddings = model(torch.tensor(dataset_celltypist[cv[i]['test']].X))
    # Apply softmax to the predictions
    # preds = torch.nn.functional.softmax(preds, dim=1).detach().cpu().numpy()
    # preds_labels = encoder_celltype.inverse_transform(preds.argmax(axis=1))
    ...
    if not os.path.exists("./results_celltypist/models/Net"):
        os.makedirs("./results_celltypist/models/Net")
    torch.save(model.state_dict(), "./results_celltypist/models/Net/model_fold_{}.pt".format(i))
    ...
    if not os.path.exists("./results_celltypist/models/logistic_regression"):
        os.makedirs("./results_celltypist/models/logistic_regression")
    joblib.dump(model, "./results_celltypist/models/logistic_regression/model_fold_{}.pkl".format(i))
    ...
    if not os.path.exists("./results_celltypist/models/KNN"):
        os.makedirs("./results_celltypist/models/KNN")
    joblib.dump(model, "./results_celltypist/models/KNN/model_fold_{}.pkl".format(i))
...
# Save results (tuple)
with open('./results_celltypist/results_celltypist.pickle', 'wb') as handle:
